...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        #self.enc_aux_loss = args.enc_aux_loss
        
        if self.selection_ratio:
            self.selection_features = Features_Selection(input_dim=self.d_model, latent_dim=128, selection_ratio=self.selection_ratio)
        else:
    ...
    def forward(self, src, mask, query_embed, pos_embed, selection_ratio):
        ...
        src, selection_loss, top_scores, mask, pos_embed = self.selection_features(src, mask, pos_embed, selection_ratio)
        tgt = torch.zeros_like(query_embed)
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed)
        hs = self.decoder(tgt, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, query_pos=query_embed)
        return hs.transpose(1, 2), selection_loss #memory.permute(1, 2, 0).view(bs, c, h, w)


class Features_Selection(nn.Module):
    def __init__(self, input_dim, latent_dim, selection_ratio):
        super().__init__()
        self.selection_ratio = selection_ratio
        self.latent_dim = latent_dim
        self.preprocess = nn.Sequential(nn.Conv2d(input_dim, input_dim, 1),
                                        nn.ReLU(),
    ...
    def forward(self, src, mask, pos_embed, sel_ratio):
        bs, c, h, w = src.shape
        print(c, h, w)
        # Scoring features for selection
        src = src.flatten(2).permute(2, 0, 1)
        selection_scores = self.autoencoder(src).permute(2, 1, 0)#.view(bs, -1)
        print(selection_scores.shape)
        selection_scores = nn.Sequential(nn.Conv2d(256, 256, 1),
                                        nn.ReLU(),
                                        nn.Conv2d(256, 1, 1),
                                        nn.ReLU()).to(torch.device('cuda'))(selection_scores).view(bs, -1)
        print(selection_scores.shape)
        ...
        selection_scores_clone = selection_scores.clone().detach()
        ...
        return sel_src, selection_loss, top_scores, sel_mask, sel_pos_embed
